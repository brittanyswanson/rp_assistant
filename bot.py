...
async def get_character_url(ctx, char_name: str):
    ...
    char_query = """SELECT name, species, player_name, url FROM characters WHERE name = %(name)s""", {
        'name': proper_character_name}
    ...
    try:
        ...
        logger.error(
            "An error happened in get_character_url using: " + char_name)
...
async def get_characters_from_player(ctx, player: str):
    ...
    char_query = """SELECT name FROM characters WHERE player_name = %(player)s AND active = 'Y' ORDER BY name ASC""", {
        'player': proper_player_name}

    data_list = query_db(char_query)

    # Check if data_list is empty
    # If empty, print out a message about no player with that name.
...
# -----------------
# Check if faceclaim is available
# -----------------


@bot.command(name='faceclaim', help='Input: None     Output: Says whether name is in use')
async def get_faceclaim(ctx, faceclaim):
    logger.info('!faceclaim was called.')
    fc_query = """SELECT count(name) FROM characters WHERE faceclaim = %(faceclaim)s AND active = 'Y'""", {
        'faceclaim': faceclaim}
